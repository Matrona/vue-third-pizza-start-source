...
export const useProfileStore = defineStore("profile", {
  ...
  getters: {
    ordersExtended: (state) => {
      ...
      return state.orders.map((order) => {
        const orderPizzas = order.orderPizzas?.map((pizza) => {
          return {
            ...
            ingredients: pizza.ingredients.map((ingredient) => {
              return {
                ...data.ingredients.find((i) => i.id === ingredient.ingredientId),
        ...
        const pizzaPrices = orderPizzas?.map((item) => item.quantity * item.price).reduce((acc, val) => acc + val, 0) ?? 0;

        const miscPrices = orderMisc?.map((item) => item.quantity * item.price).reduce((acc, val) => acc + val, 0) ?? 0;
  ...
  actions: {
    ...
    async updateAddress(address) {
      ...
      if (res.__state === "success") {
        this.addresses = this.addresses.map((i) => (i.id === address.id ? address : i));
      }
    },
  },
});
