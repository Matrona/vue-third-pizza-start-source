...
export const useDataStore = defineStore("data", {
  ...
  getters: {
    isDataLoaded() {
      return this.doughs.length > 0 && this.ingredients.length > 0 && this.sauces.length > 0 && this.sizes.length > 0 && this.misc.length > 0;
    },
  },
  actions: {
    async loadData() {
      console.log(this);
      await Promise.all([this.loadDoughs(), this.loadIngredients(), this.loadSauces(), this.loadSizes(), this.loadMisc()]);
    },
