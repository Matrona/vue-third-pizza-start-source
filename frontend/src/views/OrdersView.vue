<template>
  ...
  <section v-for="order in profileStore.ordersExtended" :key="order.id" class="sheet order">
    <div class="order__wrapper">
      ...
      <div class="order__button">
        <button type="button" class="button button--border" @click="profileStore.removeOrder(order.id)">
          Удалить
        </button>
      </div>
      <div class="order__button">
    ...
    <ul class="order__list">
      <li v-for="pizza in order?.orderPizzas" :key="pizza.id" class="order__item">
        <div class="product">
          <img :src="getPublicImage('/public/img/product.svg')" class="product__img" width="56" height="56"
            alt="Капричоза" />
    ...
    <ul v-if="order.orderMisc" class="order__additional">
      <li v-for="misc in order.orderMisc" :key="misc.id">
        <img :src="getPublicImage(misc.image)" width="20" height="30" alt="Coca-Cola 0,5 литра" />
        <p>
